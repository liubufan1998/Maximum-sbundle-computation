...
private:
	...
	char *matrix;
	ui *local_vc;
	std::vector<char> s_connected;
	std::vector<ui> temp_deleted;
	MaximumFlow *vc;
	long long branch_count;
	std::map<std::pair<ui, ui>, int> all_not_adj_inS;
	std::map<std::pair<ui, ui>, std::vector<char>> all_s_connected;
...
public:
	...
	void pure_sbundle_MultiBB(ui S_end, ui R_end, ui level, bool choose_zero, std::vector<char> &sconnected)
	{
		...
		ui branching_case = 4;
		ui min_vc = n;
		std::vector<char> s_connected2(n, 0);
	...
	void SymBD_L(ui S_end, ui R_end, ui level, bool choose_zero)
	{
		...
		if (R_end > best_solution_size && is_splex2(R_end) && vc->verify_SBundle_by_MaxFlowAlg0(SR_remap, R_end, matrix, temp_n, s, true))
	...
	{
		local_vc_backup = new ui[sizeof(ui) * ((long long)temp_n) * temp_n];
		std::copy(local_vc, local_vc + ((long long)temp_n) * temp_n, local_vc_backup);
		memset(local_vc, 0, sizeof(ui) * ((long long)temp_n) * temp_n);
	...
	{
		std::copy(local_vc_backup, local_vc_backup + ((long long)temp_n) * temp_n, local_vc);
	...
	void pure_multibranch_based_on_connetivity(ui branching_vertex_1, ui branching_vertex_2, ui S_end, ui R_end, ui level, std::vector<char> &s_connected2, ui min_vc)
	{
	...
	void generate_canselect_branches2(bool &all, ui canselect, ui &S_end, ui &R_end, ui level, ui &pos, std::vector<ui> &notadj_C, ui u1, ui u2, std::vector<char> &sconnected)
	{
		for (ui i = 0; i < canselect; ++i)
		{
	...
	void generate_the_last_branch2(bool all, ui canselect, ui &S_end, ui &R_end, ui level, std::vector<ui> notadj_C, ui u1, ui u2, std::vector<char> &sconnected)
	{
	...
	void get_notadj_S_and_notadj_C2(ui &notadj_S, std::vector<ui> &notadj_C, ui S_end, ui R_end, ui level, std::vector<char> &s_connected, ui min_vc, bool &terminate)
	{
		assert(notadj_C.empty());
	...
	void find_s_connected(std::vector<char> &s_connected, ui vertex_1, ui vertex_2)
	{
		...
		{
			s_connected = all_s_connected[std::make_pair(SR_remap[u2], SR_remap[u])];
		}
		else
		{
			s_connected = all_s_connected[std::make_pair(SR_remap[u], SR_remap[u2])];
		}
		assert(!s_connected.empty());
	}
	...
	ui get_vertex_pair_ub(ui S_end, ui R_end)
	{
		...
		for (ui i = 0; i < S_end; i++)
		{
			...
			for (ui j = i + 1; j < S_end; j++)
			{
				if (!t_matrix[SR_remap[j]])
				{
					...
					{
						all_not_adj_inS[std::make_pair(SR_remap[j], SR_remap[i])] = 1;
					}
					else
					{
						all_not_adj_inS[std::make_pair(SR_remap[i], SR_remap[j])] = 1;
					}
				}
			}
		}
		...
		double dise[t_size] = {};
		ui cost[t_size] = {};
		std::vector<std::vector<ui>> vp_partition(t_size, std::vector<ui>(R_end));
		...
		{
			std::pair<ui, ui> cur_vp = it->first;
		...
		std::vector<bool> visited(t_size, false);
		ui sum_visit = t_size;
		while (sum_visit > 0)
		{
			ui max_dise = 0;
		...
		for (ui i = 0; i < R_end; i++)
		{
			...
			for (ui j = i + 1; j < R_end; j++)
			{
				if (!t_matrix[SR_remap[j]])
				{
					...
					if (SR_remap[i] > SR_remap[j])
					{
						t_s_connected = all_s_connected[std::make_pair(SR_remap[j], SR_remap[i])];
					}
					else
					{
						t_s_connected = all_s_connected[std::make_pair(SR_remap[i], SR_remap[j])];
					}
...
private:
	...
	void SymBD_H(ui S_end, ui R_end, ui level, bool choose_zero)
	{
		...
		if (min_deg >= R_end - s)
		{
			...
			temp_n = R_end;
			SR_remap = new ui[temp_n];
			SR_remap_rid = new ui[temp_n];
			...
			assert(R_end == temp_n);
			t_level_id = new ui[temp_n];
			for (ui i = 0; i < temp_n; i++)
			{
				t_level_id[SR_remap[i]] = temp_n;
			}

			local_vc = new ui[temp_n * temp_n];
			memset(local_vc, 0, sizeof(ui) * temp_n * temp_n);
			all_s_connected.clear();
			vc->get_local_vertexConnectivity(R_end, SR_remap, matrix, temp_n, local_vc, all_s_connected, s);

			ui pre_R_end = R_end, pre_S_end = S_end;
			SymBD_L(S_end, R_end, 1, false);
			...
			if (SR_remap_rid != nullptr)
			{
				delete[] SR_remap_rid;
				SR_remap_rid = nullptr;
			}
			if (matrix != nullptr)
			{
				delete[] matrix;
