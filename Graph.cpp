...
void Graph::MSBP_SymBD_H()
{
	...
	// Randomly generate an initial s-bundle of size s for subsequent operations, since any
	// graph with no more than s vertices is a valid s-bundle
	sbundle.clear();
	for (int i = 0; i < S; i++)
	{
		sbundle.push_back(i);
	...
	// Next are our proposed three stage heuristic solution (corresponding to Algorithm 4 Degenpro in the paper)
	...
	if (sbundle.size() < UB)
	{
		...
		for (ui i = n; i > 0 && sbundle.size() < UB; i--)
		{
			...
			{							// we do this for a tradeoff of the obtained solution quality and the running time; Sure, you can adjust or remove this time according to your own needs
			...
			// The following corresponds to the operation of extracting 2-hop subgraphs based on the divide-and-conquer framework presented in our paper.
			fflush(stdout);
		...
		/***************Now start finding the exact maximum s-bundle*****************/
		Timer tt;
		...
		{
			// IE framework denotes our proposed divide-and-conquer framework
...
ui Graph::degen(ui n, ui *peel_sequence, ui *core, ept *pstart, ui *edges, ui *degree, char *vis, ListLinearHeap *heap, bool output)
{
	...
	if (new_size != 0)
	{
		...
		if (new_size - idx_splex > sbundle.size()) // means we can find a larger s-bundle
		{
			if (idx_splex == idx)
			{
				sbundle.clear();
			...
			{
				// Next, we aim to explore whether we can add a few more vertices based on the existing heuristic s-bundle
				assert(idx_splex < idx);
...
// The second heuristic stage of our three-stage heuristic algorithm
