...
namespace CTPrune
{
	...
	void orient_graph(ui n, ept m, vector<ui> &peel_sequence, ept *pstart, ept *pend, ui *edges, vector<ui> &rid)
	{
	...
	bool remove_and_shrink_oriented_tri(ui &n, ept &m, ui triangle_threshold, vector<ui> &out_mapping, vector<ui> &peel_sequence, ept *pstart, ept *pend, ui *edges, ui *tri_cnt, vector<ui> &rid, vector<ui> &degree)
	{
	...
	void truss_peeling(ui degree_threshold, ui *Qv, ui &Qv_n, ui triangle_threshold, ui *Qe, ept Qe_n, ui *tri_cnt, ept *edges_pointer, char *deleted, vector<ui> &degree, ept *pstart, ept *pend, ui *edges)
	{
	...
	void core_truss_copeeling(ui n, ui degree_threshold, ui triangle_threshold, ui *Qv, ui *Qe, ui *tri_cnt, ept *edges_pointer, char *deleted, vector<char> &exists, vector<ui> &degree, ept *pstart, ept *pend, ui *edges)
	{
		...
		while (Qe_n || Qv_n) 
		{
			while (Qe_n == 0 && Qv_n != 0) 
			{
				...
				for (ept i = pstart[u]; i < pend[u]; i++)
					deleted[i] = deleted[edges_pointer[i]] = exists[edges[i]] = 1; 
				degree[u] = 0;
	...
	void truss_pruning(ui &n, ept &m, ui triangle_threshold, vector<ui> &peel_sequence, vector<ui> &out_mapping, vector<ui> &rid, ept *pstart, ui *edges, vector<ui> &degree, bool output)
	{
		...
		ui *tri_cnt = new ui[m];
		oriented_triangle_counting(n, m, pstart, pend, edges, tri_cnt, rid.data());
		...
		reorganize_oriented_graph(n, tri_cnt, pstart, pend, pend_buf, edges, edges_pointer, rid.data());
		delete[] pend_buf;
		pend_buf = nullptr;
	...
	void core_truss_copruning(ui &n, ept &m, ui degree_threshold, ui triangle_threshold, vector<ui> &peel_sequence, vector<ui> &out_mapping, vector<ui> &rid, ept *pstart, ui *edges, vector<ui> &degree, bool output)
	{
		if (triangle_threshold == 0)
		{
			printf("!!! Triangle_threshold is 0\n");
	...
	void check_truss_pruning(ui n, ui m, ui triangle_threshold, ept *pstart, ui *edges, vector<char> &exists)
	{
		exists.clear();
		for (ui i = 0; i < n; i++)
		{
